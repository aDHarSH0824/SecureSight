...
print(f"[EMAIL CONFIG] Email notifications: {'ENABLED' if ENABLE_EMAIL_NOTIFICATIONS else 'DISABLED'}")
print(f"[EMAIL CONFIG] Admin email: {ADMIN_EMAIL}")
print(f"[EMAIL CONFIG] Mail server: {app.config['MAIL_SERVER']}:{app.config['MAIL_PORT']}")
...
try:
    with app.app_context():
        ...
        try:
            ...
            db.session.rollback()
            print(f"ℹ️ Admin user already exists or database race condition: {e}")
        
        # Create default camera setting if not exists (handling multi-worker race conditions)
        try:
...
def save_system_pid(pid):
    """Save system process PID to file"""
    try:
...
def load_system_pid():
    """Load system process PID from file"""
    try:
...
def clear_system_pid():
    """Clear system process PID file"""
    try:
        if os.path.exists(PID_FILE):
            os.remove(PID_FILE)
    except Exception as e:
...
def is_system_running():
    ...
    # Check if we have a process object and it's still running
    if system_process and system_process.poll() is None:
        return True
    
    # Check saved PID
    saved_pid = load_system_pid()
    if saved_pid:
        try:
    ...
    return len(find_main_py_processes()) > 0
...
def stop_system():
    ...
    if saved_pid:
        try:
            ...
            print(f"Error stopping saved PID process: {e}")
    
    # Stop all main.py processes
    main_processes = find_main_py_processes()
    for proc_info in main_processes:
        try:
            process = psutil.Process(proc_info['pid'])
            process.terminate()
            stopped = True
        except Exception as e:
            print(f"Error stopping process {proc_info['pid']}: {e}")
...
def start_system():
    ...
    try:
        ...
        if system_process.poll() is None:
            ...
            return False, "System failed to start"
...
def update_encodings(dataset_dirs=None, encodings_file=None):
    ...
    print(f"[INFO] Processing {len(image_paths)} images for face encoding...")
    
    for image_path in image_paths:
        name = os.path.basename(os.path.dirname(image_path))
        print(f"[INFO] Processing face: {name} from {image_path}")
    ...
    print(f"[INFO] Updated encodings.pickle with {len(known_encodings)} face encodings")
    print(f"[INFO] Known faces: {set(known_names)}")
...
def demo():
    ...
    return render_template('landing.html', scroll_to='demo')
...
def system_debug():
    try:
        ...
        # Add subprocess info if available
        global system_process
        if system_process:
            debug_info["subprocess_info"] = {
                "pid": system_process.pid,
                "poll": system_process.poll(),
                "running": system_process.poll() is None
            }
...
def toggle_system():
    try:
        current_state = is_system_running()
        if current_state:
            # Enhanced stop logic
            if stop_system():
                flash("System stopped successfully.", "info")
            else:
                flash("Failed to stop system.", "danger")
        else:
            # Enhanced start logic with error handling
            success, message = start_system()
            if success:
                flash(message, "success")
            else:
                flash(message, "danger")
    except Exception as e:
        flash(f"Error toggling system: {str(e)}", "danger")
    return redirect(url_for('dashboard'))
@app.route('/')
def index():
...
def dashboard():
    ...
    encodings_count = 0
    encodings_path = os.path.join(app.root_path, '..', '..', 'config', 'encodings.pickle')
...
def register_face():
    ...
    if request.method == 'POST':
        ...
        # Update encodings from both dataset and uploads directories
