...
known_encodings = []
known_names = []
encodings_file = "config/encodings.pickle"
last_modified_time = 0

def load_encodings():
    """Load or reload face encodings from pickle file"""
    global known_encodings, known_names, last_modified_time
...
def face_recognition_process(shm_name, shape, output_queue, cam_id, lock):
    ...
    try:
        while True:
            ...
            # Check for updated encodings every 10 frames (more frequent)
            if frame_count % 10 == 0:
                load_encodings()
            frame_count += 1
...
def save_face_frame(frame, cam_id, label):
    ...
    face_dir = os.path.join("data", "alerts", "face_alerts")
    ...
    try:
        ...
        print(f"[ERROR] Exception while saving face frame for Camera {cam_id}: {e}")
        return None
